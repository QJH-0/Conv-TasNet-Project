...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='Train Binary Conv-TasNet (BTCN)')
    ...
    print("=" * 80)
    print("BTCN Configuration loaded successfully")
    print("=" * 80)
    ...
    logger.info("=" * 80)
    logger.info("Binary Conv-TasNet (BTCN) Training")
    logger.info("=" * 80)
    
    # 创建模型
    logger.info("Creating BTCN model...")
    ...
    logger.info(f"\nSeparation (BTCN - Partially Binarized):")
    ...
    logger.info(f"  - Dropout: {config['model']['separation'].get('dropout', 0.0)}")
    logger.info(f"\nDecoder (Full Precision):")
    ...
    # 开始训练
    logger.info("\nStarting BTCN training...")
    logger.info("Note: Binary networks may converge slower and require more epochs.")
    logger.info("=" * 80)
    ...
    logger.info("=" * 80)
    logger.info("\nNext steps:")
    logger.info("  1. Check the training curves in the results directory")
    logger.info("  2. Evaluate the model using b4_evaluate.py")
    logger.info("  3. Compare with full-precision Conv-TasNet")
    logger.info("  4. (Optional) Apply knowledge distillation for better performance")
